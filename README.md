...
2. 타겟팅 단일건 발송 (특정 대상자에게 단일 메시지 발송)
   - 이벤트 발생시 즉시 발송 : 결제푸시,영수증,인증번호 등

3. 타겟팅 다중건 발송 (다수의 대상자에게 개별 메시지 발송)
   - 대량의 타겟팅 발송 혹은 발송 성능을을 위해 단일건 발송을 짧은시간 취합하여 발송 : 쿠폰, 이벤트 당첨 메시지, 개별 안내 메시지 등

4. 발송후 식별자 유효성 조회(타겟 발송 후 발송에 실패한 식별자 들에 대한 결과 조회)

5. 예약등록한 메시지 취소

* 단일건 발송이 과도하게 몰릴 결우 발송 성능이 저하 될 수 있습니다. 이러한 경우 해당 단일 건 발송들을 5 ~ 10초 이내 100 ~ 500 건 이내로 취합하여
  타겟팅 다중건 발송 API 를 이용하여 발송 하게 되면, 해당 계정에 할당된 자원을 효율적으로 이용하실 수 있습니다.
* 특정 계정에서 단일건 발송이 과도하게 호출될 경우, 서비스 이용에 제한이 있을 수 있습니다.
